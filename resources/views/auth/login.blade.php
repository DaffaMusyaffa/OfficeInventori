...
<body class="flex min-h-screen items-center justify-center bg-[#F1F5F9] p-4">
    <div class="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
        <div class="mb-6 flex flex-col items-center text-center">
            ...
            <h1 class="text-xl font-bold text-slate-800">Office Inventory<br>Request System</h1>
        ...
        <form method="POST" action="{{ route('login') }}" class="space-y-4">
            ...
            <div class="flex items-center justify-between text-sm">
                <label class="flex items-center gap-2 text-slate-600">
                    <input type="checkbox" name="remember" class="rounded border-slate-300 text-[#3B82F6] focus:ring-[#3B82F6]">
                    Ingat saya
                </label>
                <span class="text-[#3B82F6]">Lupa password?</span>
            </div>
        ...
        <p class="mt-5 text-center text-sm text-slate-500">
            Belum punya akun?
            <a href="{{ route('register') }}" class="font-semibold text-[#3B82F6] hover:underline">Daftar sebagai Employee</a>
        </p>
    </div>
